...
- Host gating now uses `cloudsmith domains list --format python
  ...
  client-side.
- Any confirmed Cloudsmith host now falls back to the released Cloudsmith
  CLI's Docker credential-helper protocol when the generic helper is
  unavailable. There is still no hardcoded host list: a CLI version that
  can't answer `domains list` trusts nothing, standard endpoints included.
  Releases are GitHub-only and installable directly from a version tag.
