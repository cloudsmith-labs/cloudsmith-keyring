...
The credential-helper contract carries no ``expires_at``, and the token
itself is opaque here: it may be a JWT with an embedded expiry, or a
non-expiring API key the user can revoke at any time. Rather than parse or
special-case either shape, callers rely on a short fixed TTL. A monotonic
