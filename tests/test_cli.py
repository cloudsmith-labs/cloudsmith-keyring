...
def test_fetch_docker_credential_uses_docker_protocol(monkeypatch):
    captured = {}

    def fake_run(binary, args, input_text=None):
        captured.update(binary=binary, args=args, input_text=input_text)
        return {"Username": "token", "Secret": "s3cr3t"}

    monkeypatch.setattr(_cli, "run_cloudsmith_json_command", fake_run)
    assert _cli.fetch_docker_credential("/usr/bin/cloudsmith", "dl.cloudsmith.io") == (
        "token",
        "s3cr3t",
    )
    assert captured == {
        "binary": "/usr/bin/cloudsmith",
        "args": ["credential-helper", "docker"],
        "input_text": "dl.cloudsmith.io\n",
    }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"Username": "token"},
        {"Secret": "s3cr3t"},
        {"Username": ["token"], "Secret": "s3cr3t"},
        {"Username": "token", "Secret": 42},
    ],
)
def test_fetch_docker_credential_rejects_invalid_payloads(monkeypatch, payload):
    monkeypatch.setattr(
        _cli, "run_cloudsmith_json_command", lambda binary, args, input_text=None: payload
    )
    assert _cli.fetch_docker_credential("/usr/bin/cloudsmith", "dl.cloudsmith.io") is None
