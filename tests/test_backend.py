...
def test_docker_credential_fallback_used_for_any_confirmed_host(monkeypatch):
    """Released CLIs lack the generic helper but still answer the Docker one."""
    monkeypatch.setattr(
        "cloudsmith_keyring.backend.find_cloudsmith_binary", lambda: "/usr/bin/cloudsmith"
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "cloudsmith_keyring.backend.fetch_docker_credential",
        lambda binary, host: ("token", "s3cr3t"),
    )
    backend = CloudsmithKeyringBackend()

    credential = backend.get_credential("https://packages.example.com/x", None)
    assert credential is not None
    assert credential.password == "s3cr3t"
...
def test_domains_lookup_unsupported_trusts_nothing(monkeypatch):
    ...
    monkeypatch.setattr(
        "cloudsmith_keyring.backend.fetch_docker_credential",
        lambda binary, host: docker_calls.append(host) or ("token", "s3cr3t"),
    )
    backend = CloudsmithKeyringBackend()
...
def _docker_stdout():
    return json.dumps({"Username": "token", "Secret": SECRET_TOKEN})
